...
interface ProdigiRepository {
    ...
    ): Flow<LoadDataStatus<List<ContentEntity>>>

    suspend fun getBannerItems(forceRefresh: Boolean): Flow<LoadDataStatus<List<BannerItem>>>

    suspend fun getWorkSheetConfig(
        id: String,
        forceRefresh: Boolean,
        byPassInitialLoading: Boolean = false,
    ): Flow<LoadDataStatus<WorkSheet>>
    ...
    suspend fun upsertSubmission(
        id: Int? = 0, workSheetId: String, name: String,
        numberId: String, className: String,
        schoolName:
        String,
        answers: List<Answer>,
    ): Int

    suspend fun submitEvaluateAnswer(
        submissionId: Int, workSheetId: String,
        submission: Submission,
    ): Flow<LoadDataStatus<Submission>>
}
